...
const Sidebar = ({isMenuOpen, project, setProject, isGenerating, setIsGenerating} : SidebarProps) => {
    ...
    if (e.key === 'Enter' && !e.shiftKey) {
        ...
        // Only submit if not generating and input isn't empty
  ...
  return (
    <div className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transition-all ${isMenuOpen ? 'max-sm:w-0 overflow-hidden' : 'w-full'}`}>
                    ...
                    if(isMessage){
                        ...
                        return(
                            <div key={msg.id} className={`flex items-start gap-3 ${isUser ? 'justify-end' : 'justify-start'}`}>
                                ...
                                {isUser && (
                                    <div className='w-8 h-8 rounded bg-grau-700 flex items-center justify-center'>
        ...
        className={`absolute bottom-2.5 right-12 p-1.5 rounded-full transition-colors ${isListening ? 'bg-red-500 animate-pulse' : 'hover:bg-gray-700'}`}>
                            {isListening ? <MicOffIcon className='size-5 text-white' /> : <MicIcon className='size-5 text-gray-400' />}
                        </button>
                </div>
            </form>
        </div>
